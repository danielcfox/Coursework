...
def answer_one():
    ...
    y_pred = np.zeros((4,100),dtype=np.float64)
    
    X_train_res = X_train.reshape(-1,1)

    i = 0
    while i < len(poly_deg):
        poly = PolynomialFeatures(degree=poly_deg[i])
        X_poly = poly.fit_transform(X_train_res)
        linreg = LinearRegression()
        linreg.fit(X_poly, y_train)
        x_pred_poly = poly.transform(x_pred)
        y_pred_poly = linreg.predict(x_pred_poly)
        y_pred[i,:] = y_pred_poly.flatten()
        i = i + 1
...
def answer_two():
    ...
    r2_train = np.zeros(max_deg+1)
    r2_test = np.zeros(max_deg+1)

    X_train_res = X_train.reshape(-1,1)
    X_test_res = X_test.reshape(-1,1)
    
    # Your code here
    for poly_deg in range(max_deg+1):
        poly = PolynomialFeatures(degree=poly_deg)
        X_train_poly = poly.fit_transform(X_train_res)
        X_test_poly = poly.fit_transform(X_test_res)
        linreg = LinearRegression().fit(X_train_poly, y_train)
        y_pred_poly_train = linreg.predict(X_train_poly)
        y_pred_poly_test = linreg.predict(X_test_poly)
        r2_train[poly_deg] = r2_score(y_train, y_pred_poly_train.flatten())
        r2_test[poly_deg] = r2_score(y_test, y_pred_poly_test.flatten())
...
def answer_four():
    ...
    linreg = LinearRegression().fit(X_train_poly, y_train)
    y_pred_poly_test = linreg.predict(X_test_poly)
    linreg_test_r2 = r2_score(y_test, (linreg.predict(X_test_poly)).flatten())
    
    lasreg = Lasso(alpha=0.01, max_iter=10000).fit(X_train_poly, y_train)
    lasreg_test_r2 = r2_score(y_test, (lasreg.predict(X_test_poly)).flatten())

    return (linreg_test_r2, lasreg_test_r2) # Your answer here
...
def answer_five():
    ...
    imp = sorted(range(len(clf.feature_importances_)), key=lambda i: clf.feature_importances_[i])[-num_imp:]
    imp.reverse()
    for i in range(num_imp):
        fi.append(X_train2.columns[imp[i]])
    return(fi) # Your answer here
...
def answer_six():
    ...
    train_scores, test_scores = validation_curve(SVC(random_state=0), X_subset, y_subset, param_name='gamma',
                                                param_range=gamma_range, cv=3, scoring="accuracy")

    train_mean = np.mean(train_scores, axis=1)
    test_mean = np.mean(test_scores, axis=1)

    return train_mean, test_mean # Your answer here
